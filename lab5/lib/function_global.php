...
function usersTable($link){
  ...
  while ($ans = mysqli_fetch_assoc($rez)){
    ...
    // Форма в столбце "Изменить пароль"
    if (!$ans['prava'] || ($ans['id'] == $_SESSION['id'])) {
      echo "<form method='post' style='margin:0;'>
				<input name='id' type='hidden' value='".$ans['id']."'/>
				<input name='password2' type='text' required/>
				<input name='CP' type='submit' value='Изменить'/>
			</form>";
    } else {
      echo "-";
    }
